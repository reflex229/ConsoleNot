...
namespace ConsoleNotLib
{
    ...
    {
        public static void CallNotification(string[] titleAndDesc, string sorryOS)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) Windows(titleAndDesc);
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)) Linux(titleAndDesc);
        ...
        private static void Linux(string[] titleAndDesc) => 
